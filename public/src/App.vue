...
const stands = ref([
  ...
  {id: 'B', name: 'Стенд B', queue: 'y', color: 'bg-gray-400'},
...
const botLink = computed(() => {
  return `https://t.me/{{sensitive data}}?start=${encodeURIComponent(telegramUsername.value)}`
})


const registerInQueue = async () => {
  try {
    const response = await axios.post("http://{{sensitive data}}:8000/queue/register", {
      title: selectedStand.value.name,
      username: telegramUsername.value,
    });
    console.log('Регистрация успешна:', response.data);
    // Обновляем информацию об очереди, если необходимо
    const res = await axios.post('http://{{sensitive data}}:8000/queue/users/count?title=' + String(selectedStand.value.name));
    const res2 = await axios.post('http://{{sensitive data}}:8000/queue/users/time?title=' + String(selectedStand.value.name));
...
const selectStand = async (stand) => {
  ...
  await axios.post("http://{{sensitive data}}:8000/queue/add", {title: selectedStand.value.name})
  const res = await axios.post('http://{{sensitive data}}:8000/queue/users/count?title=' + String(selectedStand.value.name))
  const res2 = await axios.post('http://{{sensitive data}}:8000/queue/users/time?title=' + String(selectedStand.value.name))
  queueHuman.value = res.data
  queueTime.value = res2.data
...
const leaveQueue = async () => {
  await axios.delete('http://{{sensitive data}}:8000/queue/del')
...
const login = async () => {
  if (telegramUsername.value.trim()) {
    try {
      const response = await axios.post('http://{{sensitive data}}:8000/registration?username=' + String(telegramUsername.value));
  ...
  <div class="container mx-auto px-4 py-8 max-w-md">
    <header class="flex justify-between items-center mb-8">
      <div class="flex items-center">
        <span class="text-2xl font-bold text-yellow-400">Т-БАНК</span>
      </div>
    ...
    <main>
      ...
      <div v-else-if="currentScreen === 'choose-stand'">
        <h1 class="text-2xl font-bold mb-4">Запишись, чтобы не ждать в очереди</h1>
      ...
      <div v-else-if="currentScreen === 'confirmation'" class="bg-white rounded-lg shadow-md p-6">
        <button @click="goBack" class="mb-4 text-gray-600 flex items-center hover:text-gray-800 transition-colors">
